...
job_gun.gat,86,29,3	script	クルベル	10062,{
	if(Job == Job_Rebellion) {
		if(checkquest(121910)x8) {
			if(checkquest(121925)x8 == 0) {
				setquest 121925;
				compquest 121925;
			}
			mes "[クルベル]";
...
OnStart:
	...
	for(set '@i,0; '@i<30; set '@i,'@i+1) {
		...
		set '@x,rand(127,143);
		set '@y,rand(26,29);
		makeitem 6746,1,"job_gun",'@x,'@y;
		if(rand(100) < 50) {
			set '@x,rand(127,143);
			set '@y,rand(26,29);
			makeitem 6747,1,"job_gun",'@x,'@y;
		}
		sleep 500;
	}
...
job_gun.gat,120,138,3	script	シドレン#1	10062,{
	if(CHANGE_RB == 3) {
		if(checkquest(12343)) {
			if(checkquest(12343)&4 && checkquest(12344)&4 == 0) {
				mes "[シドレン]";
				mes "ふむ、素晴らしいな！";
				mes "こうも簡単にクリアされるとは";
				mes "思っていなかったよ。";
...
job_gun.gat,220,138,4	shop	小道具自販機	564,13200
